...
const CustomerMessages = () => {
  ...
  useEffect(() => {
    ...
    const avail = myBookings.find(b => b.booking_id === bookingId);
    if (avail) { setSelectedBooking(avail); autoSelectedRef.current = true; }
  }, [conversations, myBookings, searchParams]);
  ...
  const newBookings = myBookings.filter(b => !existingBookingIds.includes(b.booking_id));
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', fontFamily: 'Manrope, system-ui, sans-serif', backgroundColor: '#f6f7f7' }}>
      ...
      <aside style={{ width: '256px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', padding: '24px 16px', backgroundColor: 'white', borderRight: '1px solid #e2e8f0', flexShrink: 0 }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
          ...
          <nav style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
            ...
            <Link to="/customer/messages" style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 12px', backgroundColor: '#1f3864', color: 'white', textDecoration: 'none', borderRadius: '8px', fontSize: '14px', fontWeight: 600 }}>
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        {/* Conversation List */}
        <div style={{ width: '320px', backgroundColor: 'white', borderRight: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
          ...
          <div style={{ flex: 1, overflowY: 'auto' }}>
            {conversations.map(conv => (
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
                  <span style={{ fontSize: '14px', fontWeight: 700, color: '#0f172a' }}>Booking #{conv.booking_id}</span>
                  <span style={{ fontSize: '12px', color: '#94a3b8' }}>{formatTime(conv.last_message_at)}</span>
                </div>
                <p style={{ fontSize: '13px', color: '#64748b', margin: '0 0 4px 0', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{conv.last_message || 'No messages'}</p>
            ...
            {newBookings.length > 0 && (
              <>
                <div style={{ padding: '12px 20px', backgroundColor: '#f8fafc', borderBottom: '1px solid #e2e8f0' }}>
                  <p style={{ fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', color: '#94a3b8', margin: 0 }}>Your bookings</p>
                </div>
                {newBookings.map(b => (
                  <div
                    key={b.booking_id}
                  ...
                  >
                    <span style={{ fontSize: '13px', fontWeight: 600, color: '#0f172a' }}>#{b.booking_id} · {b.brand} {b.model}</span>
                    <p style={{ fontSize: '12px', color: '#94a3b8', margin: '2px 0 0 0' }}>{formatDate(b.start_date)} – {formatDate(b.end_date)}</p>
                  </div>
                ))}
              </>
            )}
            ...
            {conversations.length === 0 && newBookings.length === 0 && (
              <div style={{ padding: '40px 20px', textAlign: 'center', color: '#94a3b8', fontSize: '14px' }}>No bookings to message about</div>
            )}
          </div>
        </div>
          ...
          {selectedBooking ? (
            <>
              ...
              <div style={{ padding: '16px 24px', backgroundColor: 'white', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  <h3 style={{ fontSize: '16px', fontWeight: 700, color: '#0f172a', margin: 0 }}>Booking #{selectedBooking.booking_id}</h3>
                  <p style={{ fontSize: '12px', color: '#64748b', margin: '2px 0 0 0' }}>{selectedBooking.brand} {selectedBooking.model} · {formatDate(selectedBooking.start_date)} – {formatDate(selectedBooking.end_date)}</p>
                </div>
                ...
                {messages.length === 0 && (
                  <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <div style={{ textAlign: 'center' }}>
                      ...
                      <p style={{ color: '#94a3b8', fontSize: '14px', margin: 0 }}>Send a message about your booking</p>
                    </div>
                  </div>
                )}
                {messages.map(msg => {
                  ...
                  return (
                    <div key={msg.id} style={{ display: 'flex', justifyContent: isMine ? 'flex-end' : 'flex-start' }}>
                      <div style={{ display: 'flex', gap: '8px', maxWidth: '65%', flexDirection: isMine ? 'row-reverse' : 'row', alignItems: 'flex-end' }}>
                        ...
                        <div>
                          {!isMine && <p style={{ fontSize: '11px', fontWeight: 600, color: '#64748b', margin: '0 0 4px 0' }}>{msg.sender_name} · <span style={{ textTransform: 'capitalize' }}>{msg.sender_role}</span></p>}
                          <div style={{
            ...
            <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <div style={{ textAlign: 'center' }}>
                ...
                <h3 style={{ fontSize: '18px', fontWeight: 700, color: '#64748b', margin: '0 0 8px 0' }}>Your Messages</h3>
                <p style={{ fontSize: '14px', color: '#94a3b8', margin: 0 }}>Select a booking to message the DriveEase team</p>
              </div>
            </div>
          )}
        </div>
